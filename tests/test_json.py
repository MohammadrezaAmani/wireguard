...
def test_server_json_dump_ipv4():
    ...
    calculated_server = {
        "address": ["192.168.0.5"],
        "allowed_ips": ["192.168.0.0/24", "192.168.0.5/32"],
    ...
    assert server.json(sort_keys=True) == json.dumps(
        calculated_server_w_peer, sort_keys=True
    )

    assert peer.json(sort_keys=True) == Peer(**calculated_peer).json(sort_keys=True)
    assert server.json(sort_keys=True) == Server(**calculated_server_w_peer).json(
        sort_keys=True
    )
...
def test_server_json_dump_ipv6():
    ...
    calculated_server = {
        "address": ["fde2:3a65:ca93:3125::4523:3425"],
        "allowed_ips": [
        ...
        "subnet": ["fde2:3a65:ca93:3125::/64"],
        "table": None,
    }
    calculated_peer = {
        "address": ["fde2:3a65:ca93:3125::3425:4523"],
        "allowed_ips": ["fde2:3a65:ca93:3125::3425:4523/128"],
...
def test_server_json_dump_dual_ips():
    ...
    calculated_server = {
        ...
        "subnet": ["192.168.0.0/24", "fde2:3a65:ca93:3125::/64"],
        "table": None,
    }
    calculated_peer = {
        "address": ["192.168.0.52", "fde2:3a65:ca93:3125::3425:4523"],
        "allowed_ips": ["192.168.0.52/32", "fde2:3a65:ca93:3125::3425:4523/128"],
...
def test_server_json_dump_with_nat_traversal():
    ...
    assert peer.json(sort_keys=True) == Peer(**calculated_peer).json(sort_keys=True)
    assert peer.json(sort_keys=True) == Peer(
        **json.loads(Peer(**calculated_peer).json(sort_keys=True))
    ).json(sort_keys=True)
    assert server.json(sort_keys=True) == Server(**calculated_server_w_peer).json(
        sort_keys=True
    )
    assert server.json(sort_keys=True) == Server(
        **json.loads(Server(**calculated_server_w_peer).json(sort_keys=True))
    ).json(sort_keys=True)
