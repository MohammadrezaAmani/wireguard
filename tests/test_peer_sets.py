...
def test_peer_set_removes():
    ...
    with pytest.raises(KeyError):
        server.peers.remove_by_private_key(
            "wBdB54t1rUBQ3mc0OvKdhzzaD9MvKGrshLQyHw5CN1A="
        )

    with pytest.raises(KeyError):
        server.peers.remove_by_public_key(
            "m5Tp7TvZOQYUnfmxsRN9TsmfEi5jssWpyjs5X6OP9k8="
        )
