...
@cli.command()
@click.option(
    "-e", "--endpoint", help="The public domain name for this server", prompt=True
)
@click.option(
    "-s",
    "--subnet",
    help="The subnet that this server should use for the VPN",
    prompt=True,
)
@click.option(
    "-a", "--address", help="The IP that the server should use within the VPN"
)
...
@click.option(
    "-e", "--endpoint", help="The public domain name for the VPN server", prompt=True
)
@click.option(
    "-S", "--server-pubkey", help="The public key of the VPN server", prompt=True
)
@click.option(
    "-r",
    "--routable-ip",
    help="An additional IP range that should route through the VPN",
    multiple=True,
)
@click.option(
    "-k",
    "--keepalive",
    help="How often the peer should contact the server, in seconds",
    type=int,
)
@click.option(
    "-K",
