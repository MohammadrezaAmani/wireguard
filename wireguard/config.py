...
from .utils import value_list_to_comma, value_list_to_multiple
        ...
        for peer in self._peer.peers:
            ...
            if self.preshared_key != peer.config.preshared_key:
                ...
                # The keys have both been set, but are not a match.
                    ...
                    raise ValueError(
                        f"Preshared keys do not match for {self._peer} and {peer}"
                    )
    ...
    def qrcode(self):
        ...
        if not HAS_QRCODE:
            raise AttributeError(
                "QR Code functionality is not enabled. Please add the qrcode "
